    ...
    private void allowTensThrice(List<RomanDigit> digits) {
        ...
        for (int i =1 ; i<digits.size(); i++) {
            ...
            if(++repeated == MAX_REPEAT_COUNT) {
                throw new NumberFormatException(current + " cannot repeat more than thrice, this violates the Repetition Rule for Roman Numerals");
            }
        }
    }

    private void noFivesRepeats(List<RomanDigit> digits) {
        for (int i =1 ; i<digits.size(); i++) {
            RomanDigit previous = digits.get(i-1);
            RomanDigit current = digits.get(i);
            if (current.equals(previous) && !current.canRepeat()) {
                throw new NumberFormatException(current + " cannot repeat itself, this violates the Repetition Rule for Roman Numerals");
            }
        }
    }
}
